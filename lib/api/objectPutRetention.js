...
const { ObjectLockInfo, checkUserGovernanceBypass } =
...
function objectPutRetention(authInfo, request, log, callback) {
    ...
    return async.waterfall([
        ...
        (bucket, retentionInfo, objectMD, next) => {
            ...
            const hasGovernanceBypass = bypassHeader.toLowerCase() === 'true';
            if (hasGovernanceBypass && authInfo.isRequesterAnIAMUser()) {
                return checkUserGovernanceBypass(request, authInfo, bucket, objectMD, log, err => {
                    if (err) {
        ...
        (bucket, retentionInfo, hasGovernanceBypass, objectMD, next) => {
            ...
            if (!objLockInfo.isLocked()
                || objLockInfo.canModifyPolicy(retentionInfo, hasGovernanceBypass)) {
