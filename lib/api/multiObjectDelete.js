...
const { metadataGetObject, addIsNonversionedBucket } = require('../metadata/metadataUtils');
    ...
    return async.forEachLimit(inPlay, 5, (entry, moveOn) => {
        async.waterfall([
            callback => {
                ...
                const getOptions = addIsNonversionedBucket({}, bucket);
                return callback(null, decodedVersionId, getOptions);
            },
            // for obj deletes, no need to check acl's at object level
            // (authority is at the bucket level for obj deletes)
            (versionId, getOptions, callback) => metadataGetObject(bucketName, entry.key,
                versionId, getOptions, log, (err, objMD) => {
