...
class ObjectLockInfo {
    ...
    isExtended(timestamp) {
        return (this.date === null && timestamp !== undefined)
            || (timestamp !== undefined && moment(timestamp).isAfter(this.date));
    }
    ...
    canModifyPolicy(policyChanges, hasGovernanceBypass) {
        ...
        if (this.isComplianceMode() && policyChanges.mode === 'COMPLIANCE' && this.isExtended(policyChanges.date)) {
            return true;
        }

        if (this.isGovernanceMode()) {
            // Extension are always allowed in governance mode
...
function checkUserGovernanceBypass(request, authInfo, bucketMD, objectMD, log, cb) {
    log.trace(
    ...
    const requestContextParams = {
        constantParams: {
            ...
            specificResource: { key: objectMD.getName() },
            requesterIp: ip,
