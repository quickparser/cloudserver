...
/**
  * Compute and add an isNonversionedBucket flag to options
  * @param {object} options - the current options
  * @param {object} bucketMD - bucketMD object
  * @return {object} - the updated options objects
  */
function addIsNonversionedBucket(options, bucketMD) {
    const isNonversionedBucket = bucketMD
        && bucketMD.getVersioningConfiguration() === null;

    return isNonversionedBucket
        ? Object.assign(options, { isNonversionedBucket })
        : options;
}

/** getNullVersionFromMasterMD - return metadata of null version if it exists
 * @param {object} objMD - metadata of master version
 * @param {string} bucketName - name of bucket
 * @param {string} objectKey - name of object key
 * @param {object} getOptions - options for getObjectMD
 * @param {RequestLogger} log - request logger
 * @param {function} cb - callback
 * @return {undefined} - and call callback with params err, objMD of null ver
 */
function getNullVersionFromMasterMD(objMD, bucketName, objectKey, getOptions, log, cb) {
    const options = getOptions || {};
    if (objMD.isNullKey || objMD.isNull || !objMD.versionId) {
 ...
 * NOTE: If the value of `versionId` param is 'null', this function
 * only returns the null key if it exists. If the null version is the
 * non-versioned master key or a legacy versioned key, the null
 * version object md must be retrieved in a separate step using the
 * master object md: see metadataGetNullVersionFromMaster().
...
/** metadataGetNullVersionFromMaster - retrieves the null version from
 * the master key
 *
 ...
 *
 * - the null version is stored in a versioned key (compatibility mode
 *    with old null version storage)
 *
 * @param {string} bucketName - name of bucket
 * @param {string} objectKey - name of object key
 * @param {object} getOptions - options for getObjectMD
 ...
 * @return {undefined} - and call callback with err, bucket md and object md
 */
function metadataGetNullVersionFromMaster(bucketName, objectKey, getOptions, log, cb) {
    const options = getOptions || {};
    return metadata.getObjectMD(bucketName, objectKey, options, log,
        (err, objMD) => {
            ...
            return getNullVersionFromMasterMD(objMD, bucketName, objectKey, options, log, cb);
        });
}
 ...
 * @return {undefined} - and call callback with err, bucket md and object md
 */
function metadataGetObject(bucketName, objectKey, versionId, getOptions, log, cb) {
    const options = getOptions || {};
    // may be 'null', which asks metadata to fetch the null key specifically
    options.versionId = versionId;
    return metadata.getObjectMD(bucketName, objectKey, options, log,
        (err, objMD) => {
            if (err) {
                if (err.is.NoSuchKey && versionId === 'null') {
                    return metadataGetNullVersionFromMaster(bucketName, objectKey, getOptions, log, cb);
                }
                log.debug('err getting object MD from metadata', { error: err });
                return cb(err);
            }
    ...
    async.waterfall([
        ...
        function handleNullVersionGet(bucket, objMD, next) {
            if (!objMD && versionId === 'null') {
                return metadataGetNullVersionFromMaster(bucketName, objectKey, {}, log,
