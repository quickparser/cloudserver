...
objectIngestionRate = Stat(
    ...
    reduceCalc="mean",
    targets=[Target(
        expr='sum(deriv(s3_cloudserver_objects{namespace="${namespace}", job=~"$job"}[$__rate_interval]))',  # noqa: E501
    )],
    thresholds=[
        Threshold("dark-purple", 0, 0.0),
    ],
...
bucketsCounter = Stat(
    ...
    targets=[Target(
        expr='sum(s3_cloudserver_buckets{namespace="${namespace}", job="${reportJob}"})',  # noqa: E501
    )],
    thresholds=[
        Threshold("#808080", 0, 0.0),
...
objectsCounter = Stat(
    ...
    reduceCalc="lastNotNull",
    targets=[Target(
        expr='sum(s3_cloudserver_objects{namespace="${namespace}", job="${reportJob}"})',  # noqa: E501
    )],
    thresholds=[
        Threshold("#808080", 0, 0.0),
        Threshold("blue", 1, 0.0),
...
oobObjectIngestionRate = Stat(
    ...
    reduceCalc="mean",
    targets=[Target(
        expr='sum(deriv(s3_cloudserver_ingested_objects{namespace="${namespace}", job=~"$job"}[$__rate_interval]))',  # noqa: E501
    )],
    thresholds=[
        Threshold("purple", 0, 0.0),
    ],
